/* ============================================================
   CARMON TRADEGAR · On-chain · Vista de CALOR por activo y horizonte
   Verde oscuro = acumulación · Rojo oscuro = distribución
   - Índice Carmon: tomador de decisión diario (−100..+100)
...
/* Estado (0 fr\u00edo / 1 neutral / 2 caliente) derivado de las MISMAS bandas por
   percentiles que el resto de Carmon Tradegar: el \u00edndice se traduce a temperatura y se
    ...
    <div>
      ...
      <div className="grid" style={{ gridTemplateColumns: "1.4fr 1fr 1fr", gap: 14, marginBottom: 16 }}>
        <div className="card" style={{ padding: "16px 20px", borderLeft: `6px solid ${todayCol}` }}>
          <div className="lab" style={{ fontSize: 11, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--ink-3)", display: "flex", alignItems: "center" }}>Índice Carmon · {type}-{horizon} · hoy <HelpDot k="indice" /></div>
      ...
      {/* PANEL DE CALOR */}
      <Card title={`${type}-${horizon} · Mapa de calor temporal`} sub="Precio coloreado por el Índice Carmon · banda DECISIÓN + cada métrica como banda de calor">
      ...
      {/* TABLA DE CALOR */}
      <Card title={`${type}-${horizon} · Tabla de calor`} sub={`Ordenada de la fecha más reciente a la más antigua · ${tableTruncated ? `${tableRows.length} fechas muestreadas de ${spanDays.toLocaleString("es-ES")} días` : `${tableRows.length} días consecutivos`} · DECISIÓN = Índice Carmon (verde acumular, rojo distribuir)`} style={{ marginTop: 16 }} pad={false}
      ...
      <div className="tiny muted" style={{ marginTop: 10 }}>
        {isRealBtc
          ? <>Datos reales · el <strong>Índice Carmon</strong> promedia las {metrics.length} métricas {horizon} con señal y va de −100 (distribuir) a +100 (acumular). Las métricas sin fuente (SSR, funding, Reserve Risk…) no entran aquí.</>
...
function heatExportRows(type, horizon, tableRows, metrics, rowDecision) {
  const head = ["Fecha", "Precio_USD", "Decision_IndiceCarmon Tradegar", ...metrics.map(m => shortLabel(m.label).replace(/\s+/g, "_"))];
...
function exportHeatPDF(type, horizon, tableRows, metrics, rowDecision, palette, span) {
  ...
  w.document.write(`<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><title>Carmon Tradegar · Calor ${type}-${horizon}</title>
  <style>
  ...
  <h1>🎋 Carmon Tradegar · Tabla de calor ${type}-${horizon}</h1>
  <div class="sub">${span.from} → ${span.to} · ${body.length} filas · DECISIÓN = Índice Carmon (−100 distribuir … +100 acumular) · generado ${new Date().toLocaleDateString("es-ES")}</div>
