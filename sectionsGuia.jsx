/* ============================================================
   CARMON TRADEGAR · Sección Guía — Cómo usar Carmon Tradegar (tutorial in-app)
   ============================================================ */
...
function SectionGuia({ palette }) {
  ...
  return (
    <div className="fade-in">
      <div className="page-head"><h1>Cómo usar Carmon Tradegar</h1><p>Una guía sencilla, paso a paso, para entender qué dice el mercado y tomar mejores decisiones — sin ser experto.</p></div>

      {/* qué es */}
      <Card title="¿Qué es Carmon Tradegar?" sub="Para empezar" style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 15.5, color: "var(--ink-2)", lineHeight: 1.6, margin: "0 0 12px" }}>Carmon Tradegar es un <strong>tablero que lee el mercado por ti</strong> y lo traduce a algo simple: ¿está caro o barato?, ¿es momento de comprar, vender o esperar? Por dentro mira decenas de indicadores reales de Bitcoin y Ethereum, pero tú no necesitas entenderlos todos: Carmon Tradegar hace el trabajo difícil y te da una <strong>lectura clara y una acción recomendada</strong>. Piénsalo como el <strong>termómetro y el GPS</strong> de tu inversión.</p>
        <div style={{ display: "flex", gap: 13, alignItems: "center", background: "#15201A", color: "#DCE5DD", borderRadius: 12, padding: "15px 18px" }}>
          ...
          <span style={{ fontSize: 14.5 }}>¿Ves un signo <strong style={{ color: "#fff" }}>?</strong> junto al título de cada pantalla? Pasa el cursor o tócalo y Carmon Tradegar te explica para qué sirve esa sección.</span>
        </div>
      </Card>

      {/* colores */}
      <Card title="Los 3 colores de Carmon Tradegar" sub="Lo único que debes memorizar" style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 14.5, color: "var(--ink-2)", margin: "0 0 14px" }}>Todo el tablero habla el mismo idioma de colores. Si entiendes esto, ya entiendes Carmon Tradegar:</p>
      ...
      {/* pasos */}
      <Card title="Usar Carmon Tradegar en 6 pasos" sub="Sigue este orden y tendrás una decisión completa" style={{ marginBottom: 16 }}>
        <div style={{ marginTop: -4 }}>
          ...
          <GuiaStep n="4" q="¿Cuánto invierto y cómo me protejo?" title="Arma tu Plan de operación" img="tutorial/img/4-cartera.png"
            action="nunca inviertas «todo». Deja que Carmon Tradegar te diga cuánto arriesgar (1–2% por operación) y respeta el stop.">
            En <strong>Cartera &amp; Riesgo</strong> indicas tu <strong>capital</strong> y cuánto aceptas arriesgar, y Carmon Tradegar calcula el <strong>tamaño correcto</strong> de la posición, dónde poner el <strong>stop</strong> y qué puedes ganar a cada plazo.
          </GuiaStep>
          ...
          <GuiaStep n="6" q="¿Qué pasó y qué viene esta semana?" title="Léete el Reporte 360" img="tutorial/img/6-reporte.png"
            ...
            Cada lunes, Carmon Tradegar prepara un <strong>informe de la semana</strong>: lo que pasó, las señales al cierre y lo que viene. Puedes <strong>exportarlo a PDF</strong> para guardarlo o compartirlo.
          </GuiaStep>
        </div>
      </Card>
      ...
      <div className="tiny muted" style={{ marginTop: 18, padding: "16px 0", borderTop: "1px solid var(--border)", lineHeight: 1.6, maxWidth: 720 }}>
        Carmon Tradegar es una herramienta de análisis sobre datos públicos on-chain. Sus lecturas reflejan el estado del mercado y la historia, <strong>no predicen el futuro ni constituyen asesoramiento financiero</strong>. Las decisiones de inversión son tuyas. Invierte solo lo que puedas permitirte arriesgar.
      </div>
    </div>
  );
}
