/* ============================================================
   CARMON TRADEGAR GO · Módulos — Mi cartera (G/P) + teaser backtesting
   ============================================================ */
  ...
  return (
    <div>
      ...
      <div style={{ background: "var(--card-2)", border: "1px solid var(--border)", borderRadius: 14, padding: "16px 18px" }}>
        ...
        {mode === "custom" && <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 11, lineHeight: 1.5 }}>Carmon Tradegar Go solo analiza BTC y ETH; para otras monedas registras el precio manualmente y la herramienta calcula tus ganancias y pérdidas.</div>}
      </div>
...
/* ---------- Teaser de backtesting → Carmon Tradegar + ---------- */
function BacktestTeaser({ type }) {
  ...
  const openPlus = () => { try { localStorage.setItem("bambu_page", "backtest"); } catch (e) {} window.location.href = "CarmonTradegar-Dashboard.html"; };
    ...
    <div style={{ background: "linear-gradient(150deg,#16241C,#1E3A2C)", borderRadius: 18, padding: "24px 26px", color: "#E7ECE6", boxShadow: "var(--shadow-lg)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", marginTop: 16, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,.1)" }}>
        <div style={{ flex: 1, minWidth: 220 }}>
          ...
          <div style={{ fontSize: 12.5, color: "#A8BCAD", marginTop: 2, lineHeight: 1.5 }}>Elige cualquier fecha, horizonte y estrategia y mide el resultado tú mismo. Disponible en <b style={{ color: "#EAD98A" }}>Carmon Tradegar +</b>.</div>
        </div>
        <button onClick={openPlus} style={{ cursor: "pointer", fontFamily: "var(--sans)", fontWeight: 700, fontSize: 14, padding: "13px 22px", borderRadius: 11, border: "none", background: "linear-gradient(180deg,#EAD98A,#C9A227)", color: "#26200A", boxShadow: "0 6px 18px rgba(201,162,39,.3)" }}>Abrir Carmon Tradegar + →</button>
      </div>
    </div>
  );
}
      ...
      {(() => {
        ...
        return (
          <div style={{ background: "var(--card)", borderRadius: 14, boxShadow: "var(--shadow)", padding: "18px 20px", marginBottom: 14 }}>
            ...
            <div style={{ fontSize: 13, color: "var(--ink-2)", marginBottom: 14, lineHeight: 1.55 }}>Imagina {ME.fmt.usd(st.start)} en {type} desde {st.firstIso.slice(0, 4)} ({st.years} años): siguiendo los colores de Carmon Tradegar (invertido en frío, a USD en euforia — solo {st.moves} movimientos en total) frente a comprar y no tocar:</div>
