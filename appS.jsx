/* ============================================================
   CARMON TRADEGAR GO · Decisión On-Chain
   Reutiliza el motor (BambuEngine) y los datos reales de Carmon Tradegar +.
...
/* ---------- app (shell tipo Carmon Tradegar +) ---------- */
function App() {
  ...
  const NAV = [
    ...
    { label: "Ayuda", items: [
      { id: "guia", label: "Cómo usar Carmon Tradegar Go" },
    ]},
  ];
  ...
  const TITLES = {
    ...
    guia: ["Cómo usar Carmon Tradegar Go", "Guía rápida de los elementos y cómo decidir"],
  };
  ...
  const pageGuia = (
    <div>
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
        ...
        <span style={{ flex: 1 }} />
        <a href="Como usar Carmon Tradegar Go.html" target="_blank" style={{ fontSize: 12.5, fontWeight: 700, color: "var(--brand-2)", textDecoration: "none", border: "1px solid var(--border-2)", borderRadius: 9, padding: "7px 13px", background: "var(--card)" }}>Abrir en pestaña nueva ↗</a>
      </div>
      <div style={{ border: "1px solid var(--border-2)", borderRadius: 14, overflow: "hidden", boxShadow: "var(--shadow)", background: "#EDEDE4" }}>
        <iframe src="Como usar Carmon Tradegar Go.html" title="Cómo usar Carmon Tradegar Go" style={{ display: "block", width: "100%", height: "calc(100vh - 190px)", minHeight: 520, border: "none" }} />
      </div>
    </div>
  );
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case "dca": return <><Sub>Fija tu aporte periódico y Carmon Tradegar te dice cómo soltarlo según lo caro o barato que esté: en zonas baratas, más rápido; en caras, más despacio y asegurando ganancias.</Sub><window.DcaPlan results={results} /></>;
    ...
    <div className={"sapp" + (navOpen ? " nav-open" : "")}>
      ...
      <aside className="ssidebar">
        <div className="sbrand">
          ...
          <div>
            <div className="sbrand-name">Carmon Tradegar Go{LOCK ? " · " + LOCK : ""}</div>
        ...
        <div className="snav-foot">Datos reales al {date}<br />Modelo Carmon Tradegar<br /><span style={{ color: "#566359" }}>No es asesoramiento financiero.</span></div>
      </aside>
      ...
      <div className="smain">
        <header className="stopbar">
          ...
          <div className="stb-title">{(TITLES[page] || ["Carmon Tradegar Go"])[0]}<small>{(TITLES[page] || ["", ""])[1]}</small></div>
          <span style={{ flex: 1 }} />
          {!LOCK && (
