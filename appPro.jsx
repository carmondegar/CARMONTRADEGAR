/* ============================================================
   CARMON TRADEGAR · Decisión de capital para inversor intermedio
   Reutiliza el motor de Carmon Tradegar + y responde preguntas concretas:
  ...
  return (
    <div style={{ display: "flex", gap: 12, alignItems: "flex-start", background: "var(--surface-2, #F2F6F2)", border: "1px solid var(--border)", borderRadius: 12, padding: "12px 16px", marginBottom: 16 }}>
      ...
      <div style={{ fontSize: 13, lineHeight: 1.5, color: "var(--ink-2)" }}>
        <b style={{ color: "var(--ink)" }}>Carmon Tradegar se actualiza una vez al día, después de las 13:00 UTC.</b>{" "}
...
function App() {
  ...
  const NAV = [
    ...
    { label: "Ayuda", items: [
      { id: "guia", label: "Cómo usar Carmon Tradegar" },
    ]},
  ];
  const TITLES = {
    ...
    historial: ["Historial de lecturas", "Qué habría dicho Carmon Tradegar cada día desde 2017"],
    reporte: ["Reporte 360", "Tu informe completo, semanal y on-chain"],
    guia: ["Cómo usar Carmon Tradegar", "Guía paso a paso"],
  };
  ...
  const pageSalida = (
    <div>
      <Sub>Define hoy, con la cabeza fría, a qué precios venderás parte de tu posición. El plan queda <b>guardado</b> y Carmon Tradegar lo vigila: cuando el precio alcance un nivel o el ciclo entre en zona de asegurar (más de 75° de largo plazo), aquí lo verás marcado. Vender por tramos evita la trampa de “esperar un poco más”.</Sub>
      ...
      case "guia": return (
        <div>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
            <span style={{ fontSize: 13, color: "var(--ink-2)" }}>Guía de decisión de Carmon Tradegar, con imágenes reales y explicación de cada sección.</span>
            <span style={{ flex: 1 }} />
            <a href="Como usar Carmon Tradegar.html" target="_blank" style={{ fontSize: 12.5, fontWeight: 700, color: "var(--brand-2)", textDecoration: "none", border: "1px solid var(--border-2)", borderRadius: 9, padding: "7px 13px", background: "var(--card)" }}>Abrir en pestaña nueva ↗</a>
          </div>
          <div style={{ border: "1px solid var(--border-2)", borderRadius: 14, overflow: "hidden", boxShadow: "var(--shadow)", background: "#EDEDE4" }}>
            <iframe src="Como usar Carmon Tradegar.html" title="Cómo usar Carmon Tradegar" style={{ display: "block", width: "100%", height: "calc(100vh - 190px)", minHeight: 520, border: "none" }} />
          </div>
        </div>
      );
    ...
    <div className={"sapp" + (navOpen ? " nav-open" : "")}>
      ...
      <aside className="ssidebar">
        <div className="sbrand">
          ...
          <div>
            <div className="sbrand-name">Carmon<span className="pro-badge">Tradegar</span></div>
        ...
        <div className="snav-foot">Reporte del {date} · {dataTime()}<br />Modelo Carmon Tradegar<br /><span style={{ color: "#566359" }}>No es asesoramiento financiero.</span></div>
      </aside>
      <div className="smain">
        <header className="stopbar">
          ...
          <div className="stb-title">{(TITLES[page] || ["Carmon Tradegar"])[0]}<small>{(TITLES[page] || ["", ""])[1]}</small></div>
