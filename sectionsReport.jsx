/* ============================================================
   CARMON TRADEGAR · Reporte 360 — informe semanal (lunes) + informe on-chain
...
/* Exportación en Markdown: el informe legible fuera de Carmon Tradegar, o para pasarlo
   a una IA sin perder los números. */
function weekMarkdown(w, sigs) {
  const L = [];
  L.push(`# Informe semanal on-chain · Carmon Tradegar`);
  L.push(`**Semana del ${repFecha(w.monIso)} al ${repFecha(w.end)}** · Régimen: ${w.regime} · Modelo Carmon Tradegar v2.2`);
  L.push("");
...
const CONCEPTOS = {
  ...
  lectura: { t: "la lectura de Carmon Tradegar",
...
function weekStory(w) {
  ...
  /* 2 · la lectura de Carmon Tradegar, explicada */
  blocks.push({
    ...
    p: `La lectura pasó de ${pIni.toFixed(0)} a ${pFin.toFixed(0)} de 100. ` +
      ...
      ` Carmon Tradegar mide dos plazos por separado, y esta semana no dicen lo mismo: el corto plazo cerró en ${zS.rank.toFixed(0)} (${zS.label.toLowerCase()}) y el ciclo en ${zL.rank.toFixed(0)} (${zL.label.toLowerCase()}).` +
...
function weekLessons(w) {
  ...
  out.push({
    ...
    d: "Una subida fuerte cambia el precio, no necesariamente el punto del ciclo. Por eso Carmon Tradegar mide dos plazos: uno dice si el momento es bueno para ejecutar, el otro si la fase acompaña.",
  });
      ...
      <article id="report-sheet" className="card card-pad" style={{ padding: 28, maxWidth: 1000 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, borderBottom: "2px solid var(--ink)", paddingBottom: 14, marginBottom: 18 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="tiny muted">Semana del {repFecha(w.monIso)} al {repFecha(w.end)}{addDaysIso(w.monIso, 6) > latest ? ` · en curso, ${Math.round((new Date(w.end + "T00:00:00Z") - new Date(w.monIso + "T00:00:00Z")) / 86400000) + 1} de 7 días` : ""} · Modelo Carmon Tradegar v2.2</div>
          </div>
        ...
        <div className="tiny muted" style={{ marginTop: 20, paddingTop: 12, borderTop: "1px solid var(--border)", lineHeight: 1.5 }}>
          Informe semanal generado por el modelo Carmon Tradegar sobre métricas públicas on-chain · semana {repFecha(w.monIso)} – {repFecha(w.end)}. No constituye asesoramiento financiero
        </div>
      </article>
    </div>
  );
